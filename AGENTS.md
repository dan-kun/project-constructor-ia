...
- **Agnóstico del modelo de IA**: TODA interacción con LLMs pasa por el puerto `LLMProvider`
  ...
  `claude_subscription` (CLI `Codex -p` headless con suscripción Pro/Max).
...
- [x] Robustecimiento del ciclo y del adaptador web: checkpoints para retomar una corrida interrumpida (`--resume`), límites del ciclo configurables desde `config.yaml`, subida de documentos del cliente desde el navegador, límite de tasa por IP, `pcia stats` (agregados sobre la memoria), y la suscripción de Codex habilitada en la web solo en ejecución local
